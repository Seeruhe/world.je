...
export const EnvironmentVariables = z.object({
    ...
    OPENCLAW_ENABLED: BoolAsString.optional()
        .transform((val) => toBool(val, false))
        .describe("Enable OpenClaw AI integration for chat assistant and AI NPCs. Defaults to false"),
    OPENCLAW_GATEWAY_URL: z
        .string()
        .optional()
        .describe("OpenClaw Gateway WebSocket URL (e.g., ws://openclaw-gateway:18789)"),
    OPENCLAW_API_KEY: z.string().optional().describe("API key for OpenClaw Gateway authentication"),
    OPENCLAW_DEFAULT_MODEL: z
        .string()
        .optional()
        .describe("Default AI model to use (e.g., anthropic/claude-3-5-sonnet-20241022)"),
    OPENCLAW_TRIGGER_PREFIX: z
        .string()
        .optional()
        .describe("Trigger prefix for AI chat messages (e.g., @AI)"),
    ...
    POSTGRES_HOST: z
        ...
        .optional()
        .transform(emptyStringToUndefined)
        .describe("PostgreSQL server hostname or IP address"),
    POSTGRES_PORT: PositiveIntAsString.optional()
        .transform((val) => toNumber(val, 5432))
        .describe("PostgreSQL server port. Defaults to 5432"),
    POSTGRES_USER: z
        .string()
        .optional()
        .transform(emptyStringToUndefined)
    ...
    WALLET_AUTH_ENABLED: BoolAsString.optional()
        .transform((val) => toBool(val, false))
        .describe("Enable wallet-based authentication. Defaults to false"),
    WALLET_CHAIN_ID: PositiveIntAsString.optional()
        .transform((val) => toNumber(val, 1))
        .describe("Blockchain chain ID for wallet authentication. Defaults to 1 (Ethereum mainnet)"),
    WALLET_PROJECT_ID: z
        .string()
        .optional()
        .transform(emptyStringToUndefined)
        .describe("WalletConnect project ID for mobile wallet support"),

    // Invite System Configuration
    INVITE_ENABLED: BoolAsString.optional()
        .transform((val) => toBool(val, true))
        .describe("Enable the invite system. Defaults to true"),
    INVITE_REWARD_POINTS: PositiveIntAsString.optional()
        .transform((val) => toNumber(val, 10))
        .describe("Points awarded for successful invites. Defaults to 10"),
    INVITE_CODE_LENGTH: PositiveIntAsString.optional()
        .transform((val) => toNumber(val, 8))
        .describe("Length of generated invite codes. Defaults to 8"),

    // NFT Badge Configuration
    NFT_CONTRACT_ADDRESS: z
        .string()
        .optional()
        .transform(emptyStringToUndefined)
        .describe("NFT badge contract address"),
    NFT_CHAIN_ID: PositiveIntAsString.optional()
        .transform((val) => toNumber(val, 1))
        .describe("Blockchain chain ID for NFT badges. Defaults to 1"),
});
